...
function MyAppointmentsPage(){
 ...
 if(e){setError(e.message);setLoading(false);return;}const rows=data??[];const firstRow=rows[0];if(!firstRow){setLoading(false);return;}const company=await supabase.from("companies").select("public_booking_slug").eq("id",firstRow.company_id).maybeSingle();if(company.error){setError(company.error.message);setLoading(false);return;}setBookingSlug(company.data?.public_booking_slug??"");
