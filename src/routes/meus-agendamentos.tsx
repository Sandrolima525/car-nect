...
type Appointment={id:string;customer_name:string;appointment_date:string;appointment_time:string;status:string;vehicle_plate:string|null;notes:string|null;services:string[]};
const labels:Record<string,string>={pending:"Pendente",confirmed:"Confirmado",completed:"Concluído",cancelled:"Cancelado"};

function MyAppointmentsPage(){
 const navigate=useNavigate(); const [appointments,setAppointments]=useState<Appointment[]>([]); const [email,setEmail]=useState(""); const [loading,setLoading]=useState(true); const [error,setError]=useState("");
 ...
 const {data,error:e}=await supabase.from("appointments").select("id,customer_name,appointment_date,appointment_time,status,vehicle_plate,notes").eq("customer_user_id",session.user.id).order("appointment_date",{ascending:false}).order("appointment_time",{ascending:false});
 if(e){setError(e.message);setLoading(false);return;}const rows=data??[];if(!rows.length){setLoading(false);return;}
 ...
 <CardContent className="p-4 sm:p-6"><div className="mb-5"><Button asChild className="rounded-xl"><Link to="/agendar/$slug" params={{slug:"lavapro"}}>Novo agendamento</Link></Button></div>{error&&<div className="mb-4 rounded-xl bg-destructive/10 p-3 text-sm text-destructive">{error}</div>}
