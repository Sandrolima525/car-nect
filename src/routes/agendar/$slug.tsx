...
function PublicBookingPage() {
  ...
  return <main style={brandCssVariables(themeBrand)} className="min-h-screen bg-gradient-to-br from-background via-muted/20 to-primary/[0.06] px-4 py-8 sm:py-12">
    <div className="mx-auto max-w-2xl">
      <div className="mb-8 text-center">{company.logo_url ? <img src={company.logo_url} alt={"Logo " + (company.trade_name ?? company.name)} className="mx-auto mb-3 h-20 w-20 rounded-full border-2 border-primary/20 bg-background object-cover p-1 shadow-lg ring-4 ring-primary/10" /> : <div className="mx-auto mb-3 flex h-12 w-12 items-center justify-center rounded-2xl bg-primary text-primary-foreground font-bold">LP</div>}<h1 className="text-3xl font-bold tracking-tight sm:text-4xl">{company.name}</h1><p className="mt-1 text-sm text-muted-foreground">Escolha o serviço e veja somente os horários realmente disponíveis.</p>{whatsappNumber && <Button variant="outline" size="sm" className="mt-4" asChild><a href={"https://wa.me/"+whatsappNumber} target="_blank" rel="noreferrer"><MessageCircle className="mr-2 h-4 w-4"/>Falar pelo WhatsApp</a></Button>}</div>
