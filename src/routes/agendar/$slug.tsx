  ...
  const submit = async () => {
    ...
    try {
      ...
      const { error: insertError } = await supabase.from("appointments").insert({
        company_id: company.id, service_id: serviceId, customer_name: name.trim(),
        customer_phone: phone.trim(), appointment_date: date, appointment_time: time,
  ...
  return <main className="min-h-screen bg-gradient-to-b from-background to-muted/40 px-4 py-8 sm:py-12">
    <div className="mx-auto max-w-lg">
      ...
      {done ? <Card><CardContent className="p-7 text-center">
        ...
        <p className="mt-2 text-sm text-muted-foreground">Seu pedido foi registrado. Envie a confirmação pelo WhatsApp.</p>
        ...
        <p className="mt-3 text-xs text-muted-foreground">A confirmação final depende da disponibilidade da oficina.</p>
      </CardContent></Card> :
